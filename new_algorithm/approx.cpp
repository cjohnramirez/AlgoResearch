...
SteinerTree kmb_algorithm(const Graph& graph) {
    ...
    for (auto it1 = graph.terminals.begin(); it1 != graph.terminals.end(); ++it1) {
        for (auto it2 = std::next(it1); it2 != graph.terminals.end(); ++it2) {
            ...
            // Count the number of blocked edges in the path
            int blocked_count = 0;
            ...
            terminal_edges.push_back({blocked_count, path_weight, u, v, paths[u][v].second});
        }
    }
    
    // Sort edges first by number of blocked edges, then by weight
