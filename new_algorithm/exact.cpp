...
SteinerTree exact_algorithm(const Graph& graph)
{
    ...
    int min_blocked_weight = std::numeric_limits<int>::max();
    ...
    auto calculate_metrics = [&](const std::vector<Edge> &edges) -> std::pair<int, int>
    {
        ...
        return {blocked_weight, unblocked_weight};
    };
    ...
    std::function<void(int, std::vector<Edge> &, int, int, std::vector<bool> &)> search =
        [&](int index, std::vector<Edge> &current_edges, int current_blocked_weight, int current_unblocked_weight, std::vector<bool> &connected_components)
    {
        // Early termination if we've already found a better solution
        if (current_blocked_weight > min_blocked_weight ||
            (current_blocked_weight == min_blocked_weight && current_unblocked_weight >= min_total_weight))
        {
            return;
        }
        ...
        if (index == num_edges)
        {
            ...
            {
                min_blocked_weight = current_blocked_weight;
        ...
        search(index + 1, current_edges, current_blocked_weight, current_unblocked_weight, connected_components);
        ...
        if (!creates_cycle && union_sets(edge.u, edge.v))
        {
            ...
            if (edge.is_blocked)
            {
                current_blocked_weight += edge.weight;
            }
            else
            {
            ...
            search(index + 1, current_edges, current_blocked_weight, current_unblocked_weight, new_connected_components);
