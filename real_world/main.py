...
def extract_network_data(G, terminals, blocked_edges):
    ...
    for u, v, data in G.edges(data=True):
        ...
        weight = int(
            round(data.get("length", 1.0))
        )  # Use length as integer weight, default to 1
...
def visualize_solution(
    G, terminals, blocked_edges, solution_file, output_html="solution_network.html"
):
    """
...
def visualize_network(G, terminals, blocked_edges, output_html="road_network.html"):
    ...
    try:
        ...
        for u, v, data in G.edges(data=True):
            ...
            color = (
                "red" if (u, v) in blocked_edges or (v, u) in blocked_edges else "black"
            )
...
def select_random_terminals(G, num_terminals):
    ...
    if num_terminals > len(G.nodes):
        print(
            f"Warning: Requested more terminals ({num_terminals}) than nodes ({len(G.nodes)}). Using all nodes."
        )
        return list(G.nodes())
...
def select_random_blocked_edges(G, num_blocked):
    ...
    if num_blocked > len(G.edges):
        print(
            f"Warning: Requested more blocked edges ({num_blocked}) than edges ({len(G.edges)}). Using all edges."
        )
        return list(G.edges())
...
def get_integer_input(prompt, default_value, min_value=1):
    ...
    while True:
        try:
            ...
            if value < min_value:
                print(
                    f"Value must be at least {min_value}. Using default: {default_value}"
                )
                return default_value
            return value
        except ValueError:
...
def limit_graph_size(G, max_vertices, max_edges):
    ...
    print(
        f"Created subgraph with {len(subgraph.nodes)} vertices and {len(subgraph.edges)} edges"
    )
    return subgraph
...
def main():
    try:
        ...
        print(
            f"Network downloaded: {len(G_connected.nodes)} nodes, {len(G_connected.edges)} edges"
        )

        # Ask for graph size limits
        print("\nSpecify graph size limits:")
        max_vertices = get_integer_input(
            "Maximum number of vertices", min(100, len(G_connected.nodes))
        )
        max_edges = get_integer_input(
            "Maximum number of edges", min(200, len(G_connected.edges))
        )
        ...
        num_terminals = get_integer_input(
            "Number of terminal nodes", min(5, len(G_final.nodes)), min_value=1
        )
        num_blocked = get_integer_input(
            "Number of blocked edges", min(3, len(G_final.edges)), min_value=0
        )
        ...
        if visualize_sol:
            ...
            solution_html = (
                f"{get_input('Enter solution HTML filename', 'solution_network')}.html"
            )

            # Call the solution visualization function
            visualize_solution(
                G_final, terminals, blocked_edges, solution_file, solution_html
            )
        ...
        try:
            ...
            print(
                f"Please open {os.path.abspath(output_html)} manually in your browser."
            )
